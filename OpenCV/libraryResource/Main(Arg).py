...
def main():
    ...
    nameOutputRec = sys.argv[2]
    nameOutputPlate = False

    if len(sys.argv)  == 4 :
        nameOutputPlate = sys.argv[3]
    ...
    # end if
    print(nameInput)
    imgOriginalScene  = cv2.imread(nameInput)               # open image
    ...
    if len(listOfPossiblePlates) == 0:                          # if no plates were found
        ...
        cv2.imwrite(nameOutputRec, imgOriginalScene)           # write draw Rectangle image out to file
        
        if len(sys.argv)  == 4 :
            cv2.imwrite(nameOutputPlate, licPlate.imgPlate)           # write license plate image out to file
